...
export default function Navbar() {
  ...
  return (
    <motion.header
      ...
      className={`fixed inset-x-0 top-0 z-50 border-b transition-[transform,opacity] duration-500 [transition-timing-function:cubic-bezier(0.22,1,0.36,1)] ${navVisible || open ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"} ${scrolled ? "border-white/10 bg-[#040816]/78 md:backdrop-blur-[12px]" : "border-transparent bg-transparent"}`}
    >
      ...
      <nav className="mx-auto flex w-[min(1220px,94%)] items-center justify-between px-2 py-2 md:px-0 md:py-2.5">
        ...
        <div className="hidden items-center gap-6 lg:flex">
          {links.map((item) => (
            <a key={item.id} href={`#${item.id}`} className="group editorial-link lux-interactive text-xs tracking-[0.12em] text-[#F8F4EC]/88 transition-all duration-500 hover:text-[#f8f4ec]">
              {item.label}
            </a>
          ))}
        </div>
        ...
        <div className="hidden lg:flex">
          <a href="#contact" className="lux-hover-lift lux-interactive inline-flex h-9 items-center border border-[#E2E8F0] bg-[#F8F4EC] px-5 text-xs tracking-[0.12em] text-[#111827] transition-all duration-500 hover:bg-white hover:shadow-[0_10px_22px_rgba(15,23,42,0.16)]">
            ENQUIRE NOW
          </a>
        </div>
      ...
      {open && (
        <div className="mx-auto mb-3 w-[min(1220px,94%)] border border-white/12 bg-[#040816]/92 p-5 shadow-[0_10px_24px_rgba(0,0,0,0.28)] backdrop-blur-md lg:hidden">
          <div className="flex flex-col gap-4">
            {links.map((item) => (
              <a key={item.id} href={`#${item.id}`} onClick={() => setOpen(false)} className="lux-interactive text-sm tracking-[0.1em] text-[#F8F4EC]">
                {item.label}
              </a>
            ))}
            <a href="#contact" onClick={() => setOpen(false)} className="lux-interactive mt-2 inline-flex w-fit border border-[#E2E8F0] bg-[#F8F4EC] px-4 py-2 text-sm text-[#111827]">
              ENQUIRE NOW
            </a>
          </div>
        </div>
